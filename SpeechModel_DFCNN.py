...
class ModelSpeech(): # 语音模型类
	...
	def CreateModel(self):
		...
		model.summary()
		#plot_model(modle,to_file='AM.png')
	...
	def TrainModel(self, datapath, epoch = 2, save_step = 1000, batch_size = 32, filename = abspath + 'model_speech/m' + ModelName + '/speech_model'+ModelName):
		...
		for epoch in range(epoch): # 迭代轮数
			...
			while True:
				try:
					print('[message] epoch %d . Have train datas %d+'%(epoch, n_step*save_step+base_count))
				...
				self.SaveModel(comment='_e_'+str(epoch)+'_step_'+str(n_step * save_step+base_count))
				self.TestModel(self.datapath, str_dataset='train', data_count = 64)
				self.TestModel(self.datapath, str_dataset='dev', data_count = 64)
