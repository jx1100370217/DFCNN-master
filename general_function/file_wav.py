...
def read_wav_data(filename):
	...
	#print("音频%s帧数：%d"%(audioname,num_frame))
	num_channel=wav.getnchannels() # 获取声道数
	#print("音频%s声道数：%d"%(audioname,num_channel))
	framerate=wav.getframerate() # 获取帧速率
	#print("音频%s帧速率：%d"%(audioname,framerate))
	num_sample_width=wav.getsampwidth() # 获取实例的比特宽度，即每一帧的字节数
	#print("音频%s比特宽度：%d" % (audioname, num_sample_width))
	str_data = wav.readframes(num_frame) # 读取全部的帧
	wav.close() # 关闭流
	wave_data = np.fromstring(str_data, dtype = np.short) # 将声音文件数据转换为数组矩阵形式
	#print("音频%s shape：%s" % (audioname, wave_data.shape))
	#print("音频%s：%s" % (audioname, wave_data))
	wave_data.shape = -1, num_channel # 按照声道数将数组整形，单声道时候是一列数组，双声道时候是两列的矩阵
	#print("音频%s shape（整形后）：%s" % (audioname, wave_data.shape))
	#print("音频%s（整形后）：%s" % (audioname, wave_data))
	wave_data = wave_data.T # 将矩阵转置
	#print("音频%s shape（转置后）：%s" % (audioname, wave_data.shape))
	#print("音频%s（转置后）：%s" % (audioname, wave_data))
